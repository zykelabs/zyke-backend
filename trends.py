...
# Ensure that MONGODB_URI is set in config.py or replace the placeholder with your actual URI
MONGO_URI = Config.MONGO_URI  # Example: "mongodb://localhost:27017/"
client = MongoClient(MONGO_URI)
db = client['zyke_data']        # Database name
...
async def fetch_trends_new():
    ...
    for i, trend in enumerate(descriptions):
        ...
        if unfiltered_desc == -1:
            summary, description = "N/A", "N/A"
        else:
            summary, description = extract_trend(unfiltered_desc)
        costs += cost
        final_descriptions.append([trending_searches[i], summary, description])
...
def get_trends():
    try:
        ...
        # Retrieve the most recent trends entry
        latest_trend = collection.find_one(sort=[("timestamp", -1)])

        if latest_trend:
            last_time = latest_trend.get('timestamp')
            if last_time and last_time > six_hours_ago:
                # Return the cached trends if they are less than 6 hours old
                return jsonify({"trends": latest_trend.get('trends')}), 200

        # If no trends are cached or they are older than 6 hours, fetch new trends
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        trends, costs = loop.run_until_complete(fetch_trends_new())
        loop.close()

        # Save the new trends with the current timestamp
